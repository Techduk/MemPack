...
func _connect_pending(p: PendingPeer) -> bool:
	print("Processing pending peer, TCP status:", p.tcp.get_status()) # Отладка
	if p.ws != null:
		p.ws.poll()
		var state = p.ws.get_ready_state()
		print("WebSocket state:", state) # Отладка
		if state == WebSocketPeer.STATE_OPEN:
			var id = randi_range(2, 1 << 30)
			peers[id] = p.ws
			client_connected.emit(id) # Убедись, что сигнал отправляется
			print("Client connected with ID:", id) # Отладка
			return true
		elif state != WebSocketPeer.STATE_CONNECTING:
			print("WebSocket failed, state:", state) # Отладка
			return true
		return false
	elif p.tcp.get_status() != StreamPeerTCP.STATUS_CONNECTED:
		print("TCP disconnected") # Отладка
		return true
	elif not use_tls:
		print("Creating WebSocket peer") # Отладка
		p.ws = _create_peer()
